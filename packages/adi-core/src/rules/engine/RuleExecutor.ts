...
import { ExpressionEvaluator } from "../dsl/ExpressionEvaluator";
...
export class RuleExecutor {
  ...
  execute(rule: Rule, context: RuleContext): RuleResult {
    ...
    try {
      const expressionResult = rule.expression ? this.expressionEvaluator.evaluate(rule.expression, context) : undefined;
      const traces = expressionResult?.traces ?? rule.conditions.map((condition) => this.evaluatorFactory.get(condition).evaluate(condition, context));
      const passed = expressionResult?.matched ?? traces.every((trace) => trace.matched);
